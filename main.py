...
def recommend_top_10(user_id):
    ...
    scores = R_tilde[user_idx]
    user_train_items = set(train_data[train_data['user_idx'] == user_idx]['item_idx'])
    masked_scores = scores.copy()
    masked_scores[list(user_train_items)] = -np.inf
    top_k_indices = np.argsort(masked_scores)[::-1][:10]
...
print(f"recomendation for user with id = 4: ")
print(recommend_top_10(4))
